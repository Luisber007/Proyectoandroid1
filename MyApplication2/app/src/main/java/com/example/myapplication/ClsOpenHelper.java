...
public class ClsOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
            ...
            "Tel_cliente not null, Activo text default 'Si')");

            db.execSQL("create table TblVehiculo(Placa text primary key, " +
            "Marca text not null, Modelo text not null, Valor integer not null, Activo text default 'Si')");

            db.execSQL("create table TblFactura(Cod_factura text primary key, Fecha text not null, " +
            "Ident_Cliente text not null, Activo text default 'Si', constraint pk_factura foreign key " +
            "(Ident_Cliente) references TblCliente(Ident_Cliente))");

            db.execSQL("create table TblDetalle_Factura(Cod_factura text not null, Placa text not null, " +
            "Valor_venta integer not null, constraint pk_detalle primary key (Cod_factura, Placa), " +
            "foreign key (Cod_factura) references TblFactura(Cod_factura), foreign key (Placa) references TblVehiculo(Placa))");
    }
